...
class ElectronicsTogetherPacker:
    def __init__(self, safety_margin: float = 1.1, max_electronics_volume: float = 50000, max_electronics_weight: float = 20):
    ...
    def find_optimal_box_for_group(self, products: List[Product], available_boxes: List[Box], shipping_company: Optional[str] = None) -> Optional[Box]:
        ...
        if suitable_boxes:
            return min(suitable_boxes, key=lambda x: x.width_cm * x.length_cm * (x.height_cm if not pd.isna(x.height_cm) else 2))
    ...
    def optimize_packaging_for_company(self, products: List[Product], available_boxes: List[Box], shipping_company: str) -> Optional[Dict]:
...
def categorize_box(row):
    ...
    if "envelope" in name: return "Envelope"
    elif "bag" in name: return "Bag"
    elif "roll" in name or "film" in name: return "Roll"
    elif "air" in name or "protective" in name or "bubble" in name: return "AirPack/Protective"
    elif "box" in name or "cardboard" in name or "nano" in name or "maxi" in name or "mini" in name: return "Box"
    else: return "Other"
...
def main(products_path: str,
    ...
    """
    Komut satırından verilecek yollar ve parametrelerle çalışır.
    """
    logger = get_logger()
    ...
    try:
        ...
        basket_ids = sorted(set(p.basket_id for p in products))
        logger.info("🔍 Found %d baskets (showing first %d)", len(basket_ids), min(len(basket_ids), top_n_baskets))

        for i, basket_id in enumerate(basket_ids[:top_n_baskets], start=1):
            basket_products = [p for p in products if p.basket_id == basket_id]
            electronics_count = sum(1 for p in basket_products if packer.is_electronics_product(p))
            ...
            for j, box_info in enumerate(result['boxes'], start=1):
                ...
                box_type = "📱 ELECTRONICS BOX" if box_info['is_electronics'] else "📦 REGULAR BOX"
                print(f"\n     {box_type} {j}: {box.box_name}")
                print(f"        🏢 Company: {box.shipping_company}")
                print(f"        💰 Price: {box.price} TL")
                print(f"        🎯 Utilization: {utilization:.1%}")
                print(f"        📋 Product IDs: {product_ids}")
